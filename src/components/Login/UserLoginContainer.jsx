...
import { changeLogin, changePassword, loggingIn } from "../../store/login/actions";

function UserLoginContainer(props) {
  const { login, password, changeLogin, changePassword, submitForm, showError, redirect } = props;
  return (
    <UserLogin showError={showError} login={login} password={password} redirect={redirect} changeLogin={changeLogin} changePassword={changePassword} submitForm={submitForm} />
  );
}
